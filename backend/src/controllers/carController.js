...
export const uploadCar = async (req, res, next) => {
    ...
    if (!carModel || !carYear || !carStatus || !carPrice || !carImageUrl) {
        return handlerResponse(res, 400, 'fields are missing in car info');
    }
    
    // Validate carPrice
    const priceNum = Number(carPrice);
    if (Number.isNaN(priceNum) || priceNum < 0) {
        return handlerResponse(res, 400, 'carPrice must be a non-negative number');
    }
    
    try {
        const newCar = await uploadCarService({ carModel, carYear, carStatus, carPrice: priceNum, maintenanceId, carImageUrl });
        handlerResponse(res, 201, 'Car uploaded successfully', newCar);
    } catch (error) {
        next(error);
    }
}
...
export const getAllCars = async (req, res, next) => {
    try {
        ...
        handlerResponse(res, 200, 'Cars retrieved successfully', result);
    } catch (error) {
        next(error);
    }
}
...
export const getCarById = async (req, res, next) => {
    ...
    try {
        ...
        handlerResponse(res, 200, 'Car retrieved successfully', result);
    } catch (error) {
        next(error);
    }
}
...
export const updateCarByID = async (req, res, next) => {
    try {
        ...
        handlerResponse(res, 200, 'Car updated successfully', updatedCar);
    } catch (error) {
        next(error);
    }
};
...
export const searchCars = async (req, res, next) => {
    try {
        ...
        if (year && (Number.isNaN(Number(year)) || Number(year) <= 0)) {
            return handlerResponse(res, 400, 'year must be a positive number');
        }
        if (yearMin && (Number.isNaN(Number(yearMin)) || Number(yearMin) <= 0)) {
            return handlerResponse(res, 400, 'yearMin must be a positive number');
        }
        if (yearMax && (Number.isNaN(Number(yearMax)) || Number(yearMax) <= 0)) {
            return handlerResponse(res, 400, 'yearMax must be a positive number');
        }
        if (priceMin && (Number.isNaN(Number(priceMin)) || Number(priceMin) < 0)) {
            return handlerResponse(res, 400, 'priceMin must be a non-negative number');
        }
        if (priceMax && (Number.isNaN(Number(priceMax)) || Number(priceMax) < 0)) {
            return handlerResponse(res, 400, 'priceMax must be a non-negative number');
        }
        if (page && (Number.isNaN(Number(page)) || Number(page) <= 0)) {
            return handlerResponse(res, 400, 'page must be a positive number');
        }
        if (limit && (Number.isNaN(Number(limit)) || Number(limit) <= 0)) {
            return handlerResponse(res, 400, 'limit must be a positive number');
        }
        ...
        return handlerResponse(res, 200, 'Cars retrieved successfully', result);
    } catch (error) {
        if (error.message.includes('Invalid status') || error.message.includes('Invalid sort')) {
            return handlerResponse(res, 400, error.message);
        }
        next(error);
    }
};